...
import {
  PICX_CONFIG,
  PICX_MANAGEMENT,
  PICX_UPLOADED, UserConfigInfoModel
...
export default createStore({
  ...
  mutations: {
    // 设置用户配置信息
    SET_USER_CONFIG_INFO(state: any, configInfo) {
    ...
    // 用户配置信息 - 增加目录
    USER_CONFIG_INFO_ADD_DIR({commit, state}, dir) {
      if (!state.userConfigInfo.dirList.some((v: any) => v.value === dir)) {
        state.userConfigInfo.dirList.push({label: dir, value: dir})
        commit('PERSIST_USER_CONFIG_INFO')
      }
    },

    // 用户配置信息 - 删除目录列表的某个目录
    USER_CONFIG_INFO_REMOVE_DIR({commit, state}, dir) {
  ...
  actions: {
    ...
    UPLOADED_LIST_REMOVE({state, dispatch}, item) {
      if (state.uploadedImageList.length > 0) {
        const rmIndex = state.uploadedImageList.findIndex((v: any) => v.uuid === item.uuid)
        if (rmIndex !== -1) {
